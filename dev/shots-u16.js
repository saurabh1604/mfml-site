/* Unit 16 screenshots WITH WebGL: node shots-u16.js [width=1440] [theme=dark] [ids=hero,s1,w-neuron,…|all] [waitMs]
   → shots/u16/<width>-<theme>-<id>-<k>.png (tall targets are cut into viewport chunks). Prints console errors + overflow.
...
const OUT = __dirname + '/shots/u16/'; fs.mkdirSync(OUT, { recursive: true });
(async () => {
  ...
  await page.goto('file:///home/claude/mfml-site/dev/site/unit-16.html');
  ...
  for (const id0 of ids) {
    ...
    if (id === 'hero' && at) {
      ...
      await page.evaluate(a => { window.U16HeroAt = a; }, +at); await page.waitForTimeout(1600);
