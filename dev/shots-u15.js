/* Unit 15 screenshots WITH WebGL: node shots-u15.js [width=1440] [theme=dark] [ids=hero,s1,w-neuron,…|all] [waitMs]
   → shots/u15/<width>-<theme>-<id>-<k>.png (tall targets are cut into viewport chunks). Prints console errors + overflow.
...
const OUT = __dirname + '/shots/u15/'; fs.mkdirSync(OUT, { recursive: true });
(async () => {
  ...
  await page.goto('file:///home/claude/mfml-site/dev/site/unit-15.html');
  ...
  for (const id0 of ids) {
    ...
    if (id === 'hero' && at) {
      ...
      await page.evaluate(a => { window.U15HeroAt = a; }, +at); await page.waitForTimeout(1600);
