/* Unit 18 screenshots WITH WebGL: node shots-u18.js [width=1440] [theme=dark] [ids=hero,s1,w-qkv,…|all] [waitMs]
   → shots/u18/<width>-<theme>-<id>-<k>.png (tall targets are cut into viewport chunks). Prints console errors + overflow.
...
const OUT = __dirname + '/shots/u18/'; fs.mkdirSync(OUT, { recursive: true });
(async () => {
  ...
  const VH = +(process.argv[6] || (W < 600 ? 844 : 900));
  ...
  for (const id0 of ids) {
    ...
    if (id === 'hero' && at) {
      ...
      await page.evaluate(a => { window.U18HeroAt = a; }, +at); await page.waitForTimeout(1600);
