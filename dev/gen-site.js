...
const THREADS = [
  { id: 'dot', name: 'Agreement', lede: 'One number that says how much two lists agree — it measures angles, casts shadows, points the gradient, and finally decides what a transformer pays attention to.',
    stops: [[1, 's6', 'rows as dot products'], [3, 's2d', 'the dot product'], [3, 's6', 'angles'], [3, 's10', 'projection'], [6, 's6', 'the gradient compass'], [11, 's14', 'the widest street'], [13, 's12', 'the classifier as a vote'], [16, 's11', 'three rulers'], [18, 's3', 'attention scores']] },
  { id: 'eig', name: 'Directions a matrix leaves alone', lede: 'Eigenvectors keep turning up: they unpack a matrix, judge a landscape, explain why a valley is slow, and name the directions that matter in data.',
    stops: [[4, 's5', 'eigenvectors'], [4, 's7', 'spectral theorem'], [5, 's7', 'the SVD'], [8, 's10', 'the Hessian judge'], [10, 's8', 'the canyon'], [11, 's6', 'the stiffness dial'], [12, 's5', 'principal components'], [14, 's5', 'the Gaussian ellipse'], [17, 's6', 'memory that fades']] },
  { id: 'null', name: 'What the machine cannot see', lede: 'Some inputs vanish without a trace. The blind spot of a matrix explains missing solutions, redundant features and why a model can memorise.',
    stops: [[1, 's10', 'the null space'], [1, 's13', 'rank'], [2, 's5', 'a subspace'], [2, 's11', 'dimension'], [5, 's10', 'low rank'], [10, 's3', 'knobs versus facts'], [12, 's13', 'the Gram trick'], [16, 's5', 'the SVD squeeze']] },
  { id: 'chain', name: 'Blame flowing backwards', lede: 'The chain rule, promoted step by step: a product of slopes, a product of Jacobians, a sweep backwards through a graph — and later, through time.',
    stops: [[6, 's10', 'chain rule as matrices'], [7, 's3', 'two rules'], [7, 's6', 'a whole layer'], [7, 's8', 'one sweep'], [15, 's8', 'the whole network'], [15, 's10', 'blame that fades or explodes'], [17, 's5', 'backprop through time']] },
...
const units = [];
for (let n = 1; n <= 20; n++) { if (fs.existsSync(path.join(SRC, 'unit-' + String(n).padStart(2, '0') + '.html'))) units.push(parseUnit(n)); }
