...
(async () => {
  ...
  for (const u of UNITS) {
    ...
    if (tocLinks !== secCount + (u === 1 || u === 18 ? 2 : 3)) {
    ...
    const wantPrev = u > 1 ? `Unit ${u - 1}` : null, wantNext = u < 18 ? `Unit ${u + 1}` : null;
    ...
    if (u === 18 && navText.includes('Unit 19')) fail(tag, 'unit 18 offers a nonexistent unit 19');
