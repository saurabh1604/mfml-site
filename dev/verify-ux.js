...
const UNITS = process.env.ONLY ? process.env.ONLY.split(',').map(Number) : [1, 2, 3, 4, 5, 6, 7, 8, 9];
const EXPECT_CHECKS = { 1: 11, 2: 11, 3: 13, 4: 13, 5: 17, 6: 17, 7: 16, 8: 17, 9: 15 };
const EXPECT_PROBS = { 1: 10, 2: 18, 3: 8, 4: 5, 5: 12, 6: 10, 7: 11, 8: 12, 9: 12 };
...
(async () => {
  ...
  for (const u of UNITS) {
    ...
    if (tocLinks !== secCount + (u === 1 || u === 9 ? 2 : 3)) {
    ...
    const wantPrev = u > 1 ? `Unit ${u - 1}` : null, wantNext = u < 9 ? `Unit ${u + 1}` : null;
    ...
    if (u === 9 && navText.includes('Unit 10')) fail(tag, 'unit 9 offers a nonexistent unit 10');
  ...
  console.log('\n— index (hub) —');
  {
    ...
    const want = ['13 widgets · 11 checks · 10 problems', '6 widgets · 11 checks · 18 problems',
                  ...
                  '7 widgets · 16 checks · 11 problems', '9 widgets · 17 checks · 12 problems', '11 widgets · 15 checks · 12 problems'];
    ...
    if (overall !== '15 of 130 checks passed') fail('hub', `overall reads "${overall}", expected "15 of 130 checks passed"`);
    const barW = await page.evaluate(() => document.getElementById('obar-fill').style.width);
    if (barW !== '11.5%') fail('hub', `overall bar width ${barW}, expected 11.5%`);
