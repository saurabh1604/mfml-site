/* Unit 13 screenshots WITH WebGL: node shots-u13.js [width=1440] [theme=dark] [ids=hero,s1,…|all] [mode=section|widget]
   → shots/u13/<width>-<theme>-<id>-<k>.png (tall shots are cut into ≤1400px chunks). Prints console errors. */
const { chromium } = require('playwright');
const fs = require('fs');
const W = +(process.argv[2] || 1440), THEME = process.argv[3] || 'dark', IDS = process.argv[4] || 'all', MODE = process.argv[5] || 'section';
const OUT = __dirname + '/shots/u13/'; fs.mkdirSync(OUT, { recursive: true });
(async () => {
  ...
  await page.goto('file:///home/claude/mfml-site/dev/site/unit-13.html');
