/* ================= UNIT 17 · local helpers (carried over from Units 11, 13 and 15) (on top of the house runtime) ================= */
...
function starfield(ctx,n,R){ const {THREE,root}=ctx; if(ctx.isLight) return null; const sp=new Float32Array(n*3);
  ...
  for(let k=0;k<n;k++){ let x,y,z; do{ x=(rnd()*2-1)*R*1.3; y=(rnd()*2-1)*R*.6; z=-2-rnd()*R*1.2; }while(Math.hypot(x,y-1,z)<R*.3);   /* only behind the scene: no star floats in front of the camera */ sp[3*k]=x; sp[3*k+1]=y; sp[3*k+2]=z; }
