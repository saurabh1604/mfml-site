...
/* ---------- §7 · w-clip: a loss cliff, with and without the speed governor (3-D) ---------- */
(function(){
  ...
  function readout(){ const {P,off}=run(), big=P.reduce((m,s,i)=>s.gn>P[m].gn?i:m,1), last=P[P.length-1], n=Math.min(st.n,P.length-1), cur=P[n];
    read.innerHTML='Step '+n+' of '+(P.length-1)+' · loss <b>'+N(cur.L,3)+'</b>'+(n>0?' · gradient length <b class="'+(cur.gn>st.c?'r':'m')+'">'+N(cur.gn,2)+'</b>'+(cur.clipped?' → clipped to <b class="g">'+N(st.c,2)+'</b>':''):'')+
      ...
      '<br>'+(off?'<b class="r">Flung off the map</b> — one step undid everything.':'<b class="m">Reached the valley</b>: loss '+N(last.L,4)+' after '+(P.length-1)+' steps.');
  ...
  function build(){ return CIN.stage3d(box,{camera:cam3({pos:[-4.5,5.0,8.3],look:[.2,.75,0],fov:38},{pos:[-6.2,6.4,11.2],look:[.1,.7,0],fov:40}),orbit:true,autoRotate:.08,autoRotateStopsOnUser:true,
    build(ctx){ const {THREE,root,isLight,colors}=ctx, hx=hxOf(ctx), c=colors;
      ...
      root.add(lab(ctx,'the cliff',M3(CL.A0-.05,-1.7,CL.H*ZS*.5+.35),{size:24,weight:700,scale:.0095,color:c.ink2,bg:true,depthTest:false}));
      root.add(lab(ctx,'valley',M3(-1.8,-2.4,.7),{size:22,weight:700,scale:.0095,color:c.ink2,bg:true,depthTest:false}));
      root.add(lab(ctx,'start',M3(CL.start[0],CL.start[1],Lf(...CL.start)*ZS+.45),{size:20,weight:700,scale:.0095,color:c.s1,bg:true,depthTest:false}));
      ...
      ctx.redraw=()=>{ clearGroup(dyn); const {P}=run(), n=Math.min(st.n,P.length-1);
        ...
        for(let i=1;i<=n;i++){ const A=at(P[i-1]), B=at(P[i]), s=P[i], big=s.gn>1.5; const col=s.clipped?hx(K17.gate):big?critHex():hx(K17.word);
          const mid=[(A[0]+B[0])/2,Math.max(A[1],B[1])+(big?.9:.15),(A[2]+B[2])/2];
        ...
        if(Math.abs(P[n].p[0])>CL.R||Math.abs(P[n].p[1])>CL.R){ dyn.add(lab(ctx,'flung off the map!',[tip[0]-.2,tip[1]+1.0,tip[2]+.4],{size:26,weight:800,scale:.0095,color:cssv('critical'),bg:true})); }
        RR(ctx); };
      ctx.redraw();
    }}); }
  ...
  document.getElementById('cl-reset').addEventListener('click',()=>{ clearInterval(timer); st.n=0; redraw(); });
  readout(); window.U17Clip={st,run,redraw,Lf,gf,CL};
})();

/* ---------- §8 · w-notebook: the cell state as a tank with three valves ---------- */
(function(){
  const svg=document.getElementById('nb-svg'); if(!svg) return;
  const read=document.getElementById('nb-read');
  const W0={c:1,f:.9,i:.2,g:.5,o:1};
  const st=Object.assign({ph:1},W0);          /* ph: 0 = old level shown, ½ = after the eraser, 1 = after the pen */
  function draw(){ svg.innerHTML=''; const V=narrowSVG(svg,560,340,440,430), ph=V.n, glow=glo(svg), keep=st.f*st.c, add=st.i*st.g, c=keep+add, h=st.o*tanh(c);
    ...
    for(let v=-2;v<=2;v++){ const y=ly(v); el('line',{x1:TX,y1:y,x2:TX+10,y2:y,stroke:'var(--ink-muted)'},svg); txt(svg,TX-6,y+4,nm(String(v)),'font:500 10px system-ui;fill:var(--ink-muted)','end'); }
    ...
    txt(svg,TX+TW+8,ly(st.c)+4,(ph?'':'old ')+N(st.c,2),'font:600 10.5px system-ui;fill:var(--ink-2)');
    txt(svg,TX+TW/2,yl+(shown>=0?-8:18),'c = '+N(shown,3),'font:800 13px system-ui;fill:var(--ink)','middle');
    ...
    txt(svg,NX,WY-62,'note h','font:700 11px system-ui;fill:var(--ink-2)','middle'); txt(svg,NX,WY+70,N(h,4),'font:800 12px system-ui;fill:'+cv(K17.mem),'middle');
    read.innerHTML='c = f·c<sub>old</sub> + i·g = '+N(st.f,2)+' · '+N(st.c,2)+' + '+N(st.i,2)+' · '+N(st.g,2)+' = '+N(keep,4)+' + '+N(add,4)+' = <b class="m">'+N(c,4)+'</b><br>h = o·tanh(c) = '+N(st.o,2)+' · tanh('+N(c,4)+') = <b class="m">'+N(h,4)+'</b>'+
      (st.f===1&&st.i===0?'<br><b class="g">Hold:</b> nothing erased, nothing written — the level is exactly the old one.':'');
    svg.dataset.c=c.toFixed(6); svg.dataset.h=h.toFixed(6); mfont(svg); }
  ['c','f','i','g','o'].forEach(k=>bindCtl('nb-'+k,v=>{ st[k]=v; st.ph=1; draw(); },v=>N(v,2)));
  const setAll=o=>{ Object.assign(st,o); ['c','f','i','g','o'].forEach(k=>setCtl('nb-'+k,st[k],v=>N(v,2))); };
  ...
  document.getElementById('nb-reset').addEventListener('click',()=>{ setAll(W0); st.ph=1; draw(); });
  draw(); new MutationObserver(draw).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']}); onResize(draw);
  window.U17Notebook={st,draw};
})();

/* ---------- §8 · w-lstm: THE MEMORY LAB — race a plain cell, a GRU and an LSTM (3-D + charts) ---------- */
(function(){
  ...
  function chart(){ svg.innerHTML=''; const VB=narrowSVG(svg,760,280,440,330), ph=VB.n, glow=glo(svg), W=VB.W,H=VB.H,L=ph?50:56,Rr=ph?14:150,Tp=ph?68:22,B=36, R=run();
    ...
    if(st.tab==='acc'){ const D=acc(), px=T=>L+(T-5)/45*(W-L-Rr), py=v=>Tp+(1-v)/(1-.3)*(H-Tp-B);
      for(let v=.3;v<=1.001;v+=.1){ const y=py(v); el('line',{x1:L,y1:y,x2:W-Rr,y2:y,stroke:'var(--grid)'},svg); txt(svg,L-6,y+3.5,Math.round(v*100)+'%','font:500 10px system-ui;fill:var(--ink-muted)','end'); }
      const yh=py(.5); el('line',{x1:L,y1:yh,x2:W-Rr,y2:yh,stroke:'var(--ink-muted)','stroke-dasharray':'5 4'},svg); txt(svg,ph?L+4:W-Rr-4,yh+(ph?14:-5),'coin toss','font:600 10px system-ui;fill:var(--ink-muted)',ph?'start':'end');
      LAB.lengths.filter(T=>!ph||T%10===0||T===5).forEach(T=>txt(svg,px(T),H-B+15,String(T),'font:500 10px system-ui;fill:var(--ink-muted)','middle')); txt(svg,(L+W-Rr)/2,H-6,'sentence length (words)','font:600 10.5px system-ui;fill:var(--ink-muted)','middle');
      el('line',{x1:px(st.T),y1:Tp,x2:px(st.T),y2:H-B,stroke:cv(K17.gate),'stroke-width':1.4,'stroke-dasharray':'3 3'},svg);
      CELLS.forEach((C,j)=>{ const pts=D.map(d=>[px(d.T),py(d[C.id])]); const g=el('g',glow?{filter:glow}:{},svg);
      ...
      txt(svg,ph?8:L,ph?42:14,ph?'right answers, of 400 sentences':'share of 400 random sentences answered right','font:600 10.5px system-ui;fill:var(--ink-muted)');
      ...
      for(let e=0;e>=LO;e-=2){ const y=py(10**e); el('line',{x1:L,y1:y,x2:W-Rr,y2:y,stroke:'var(--grid)'},svg); txt(svg,L-6,y+3.5,e===0?'1':'10'+SUP(e),'font:500 10px system-ui;fill:var(--ink-muted)','end'); }
      for(let s=0;s<T;s+=Math.max(1,Math.round((T-1)/(ph?5:10)))) txt(svg,px(s),H-B+15,String(s),'font:500 10px system-ui;fill:var(--ink-muted)','middle');
      txt(svg,(L+W-Rr)/2,H-6,'steps back from the end','font:600 10.5px system-ui;fill:var(--ink-muted)','middle');
      CELLS.forEach((C,j)=>{ const bl=R.bl[C.id]; const pts=[]; for(let t=T;t>=1;t--) pts.push([px(T-t),py(bl[t])]); const g=el('g',glow?{filter:glow}:{},svg);
        ...
        const v1=bl[1]; legend(C,j,E(v1,2)); if(!ph) txt(svg,W-Rr+10,py(v1)+4,C.name+' '+E(v1,2),'font:700 11px system-ui;fill:'+cv(C.col)); });
...
(function(){
  ...
  function draw(){ svg.innerHTML=''; narrowSVG(svg,560,360,440,400); const p=plane(svg,-1.25,1.25,-1.1,1.1,{pad:20,step:.5,noticks:true}), glow=p.glow;
    [-1,-.5,.5,1].forEach(v=>{ txt(svg,p.px(v),p.py(0)+14,nm(String(v)),'font:500 10px system-ui;fill:var(--ink-muted)','middle'); txt(svg,p.px(0)-6,p.py(v)+3.5,nm(String(v)),'font:500 10px system-ui;fill:var(--ink-muted)','end'); });
    ...
    arrow(p,0,0,H0[0],H0[1],cv(K17.mem),2.8,'old note h','font:700 12px system-ui;fill:'+cv(K17.mem));
    arrow(p,0,0,hc[0],hc[1],cv(K17.word),2.8,'candidate h̃','font:700 12px system-ui;fill:'+cv(K17.word));
    arrow(p,0,0,hn[0],hn[1],cv(K17.gate),3.2,'new note','font:800 12px system-ui;fill:'+cv(K17.gate));
    glowDot(svg,p.px(hn[0]),p.py(hn[1]),5,cv(K17.gate),glow);
    /* the dial */
    const DX=svg.viewBox.baseVal.width-62, DY=PHONE()?52:60, Rd=PHONE()?28:34; el('circle',{cx:DX,cy:DY,r:Rd,fill:'var(--surface-2, var(--surface))',stroke:'var(--line)'},svg);
    ...
    txt(svg,DX,DY+Rd+16,'z = '+N(st.z,2),'font:800 12px system-ui;fill:'+cv(K17.gate),'middle'); txt(svg,DX-Rd-2,DY+Rd+2,'old','font:600 9.5px system-ui;fill:var(--ink-muted)','end'); txt(svg,DX+Rd+2,DY+Rd+2,'new','font:600 9.5px system-ui;fill:var(--ink-muted)');
    ...
    svg.dataset.h=hn.map(v=>v.toFixed(6)).join(','); mfont(svg); }
  ...
  draw(); new MutationObserver(draw).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']}); onResize(draw);
  window.U17GRU={st,draw,cand,blend};
})();

/* ---------- §9 · w-gates-compare: three cells side by side ---------- */
(function(){
  const svg=document.getElementById('gc-svg'); if(!svg) return;
  const read=document.getElementById('gc-read');
  const CELL=[{id:'rnn',name:'plain cell',blocks:['h̃ · tanh'],k:1,eq:'h = tanh(W h + U x + b)'},
  ...
  function draw(){ svg.innerHTML=''; const VB=narrowSVG(svg,760,300,440,590), ph=VB.n, glow=glo(svg), P=PARAMS(st.h,st.d), max=P.lstm;
    CELL.forEach((C,j)=>{ const x0=4, sel=st.sel===C.id; const g=el('g',{style:'cursor:pointer','data-cell':C.id,transform:ph?`translate(${[0,220,110][j]},${[0,0,296][j]})`:`translate(${26+j*250},0)`},svg);
      ...
      el('rect',{x:x0+15,y:180,width:190,height:22,rx:6,fill:'none',stroke:'var(--line)'},g); el('rect',{x:x0+15,y:180,width:bw,height:22,rx:6,fill:cv(K17.gate),opacity:.85},bg);
