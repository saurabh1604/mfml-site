...
/* ---------- §10 · w-seq2seq: English in, one summary orb, Hindi out (3-D) ---------- */
(function(){
  ...
  const SLOTS=8, KEEP=.9, SHOW=12;
  const st={n:6,tf:false,ph:1};
  const word=k=>EN[k%EN.length];
  ...
  function build(){ if(!CIN) return null; const P=PHONE();
    ...
    build(ctx){ const {THREE,root,isLight,colors}=ctx, hx=hxOf(ctx), dark=!isLight, SC=P?.0122:.0088;
      ...
      const OX=0, OY=P?1.5:1.35, OR=P?.58:.62, EY=P?3.05:OY, DY=P?-.05:OY, SH=P?7:SHOW;
      ...
      root.add(lab(ctx,P?'summary · 8':'summary · 8 numbers',P?[OX+1.55,OY+.1,0]:[OX,OY+OR+.45,0],{size:26,weight:800,scale:SC,color:colors.s3,bg:true}));
      if(!P){ root.add(lab(ctx,'encoder · reads English',[-3.3,2.45,0],{size:24,weight:700,scale:SC,color:colors.muted,bg:false}));
        root.add(lab(ctx,'decoder · writes Hindi',[3.9,2.45,0],{size:24,weight:700,scale:SC,color:colors.muted,bg:false})); }
      ...
      ctx.redraw=()=>{ clearGroup(dyn); const n=st.n, ph=st.ph, shown=Math.min(n,SH);
        ...
        for(let i=0;i<shown;i++){ const k=(n>SH&&i>=half)?n-(shown-i):i, on=encPh*shown>=i+.5; const x=encX(i);
          ...
          dyn.add(lab(ctx,word(k),P?[x,EY+.55,0]:[x,.42,0],{size:shown>8?22:27,weight:700,scale:SC,color:colors.s1,bg:true}));
        ...
        for(let k=0;k<n;k++){ const arrived=encPh*n>=k+.5; if(!arrived) continue; const s=Math.pow(KEEP,n-1-k);
          ...
          if(k===0){ const hh=haloSprite(ctx,hx(K17.gate),.2+.9*s); hh.position.set(...p); dyn.add(hh); dyn.add(lab(ctx,'word 1 · '+E(s,2),P?[OX-1.6,OY+.1,0]:[p[0],p[1]+.28,p[2]],{size:20,weight:700,scale:SC,color:colors.s4,bg:true})); } }
...
/* ---------- §11 · w-beam: greedy against beam search ---------- */
(function(){
  ...
  function draw(){ svg.innerHTML=''; const VB=narrowSVG(svg,760,360,440,610), ph=VB.n, glow=glo(svg), R=beamSearch(st.k), s1=R.log[0].kept, s2=R.log[1].kept, best=R.best.w.join(' ');
    ...
    svg.dataset.best=best+'|'+R.best.p.toFixed(4); svg.dataset.k=st.k; mfont(svg); }
